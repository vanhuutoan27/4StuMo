...
const Navigation = () => {
  ...
  return (
    <div className="absolute inset-x-0 bottom-0 rounded-t-xl bg-card text-gray-400 shadow-md">
      <div className="flex items-center justify-between text-xs">
        {navItems.map((navItem) => {
          ...
          return (
            ...
            >
              <navItem.icon
                className={`cursor-pointer ${isAddButton ? 'h-8 w-8' : ''}`}
              />
              {isAddButton ? (
                ''
              ) : (
                <span className="mt-2 text-[11px]">{navItem.name}</span>
              )}
            </NavLink>
          );
        })}
